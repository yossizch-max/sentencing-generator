...
const intExpected=v=>/^\d+$/.test(String(v??''))?Number(v):null;
...
if(setResult.actual.ps67s_prior_67_count!==undefined && combined.currentOffense.routeFacts.section67.prior67Count!==intExpected(setResult.actual.ps67s_prior_67_count)) throw new Error('67 prior mismatch '+JSON.stringify({setResult,model:combined.currentOffense.routeFacts.section67}));
...
if(setResult.actual.ps67s_prior_shich_count!==undefined && combined.currentOffense.routeFacts.intoxication.priorCount!==intExpected(setResult.actual.ps67s_prior_shich_count)) throw new Error('shichrut prior mismatch');
...
if(setResult.actual.acc_victim_count!==undefined && acc.accident.victimCount!==intExpected(setResult.actual.acc_victim_count)) throw new Error('accident victim count mismatch');
