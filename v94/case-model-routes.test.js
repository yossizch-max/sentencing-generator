...
for(const route of routes){
  ...
  if(route==='accident_injury'||route==='accident_below_real'){map.acc_injury_level=mkEl('hard');map.acc_negligence=mkEl('high');map.acc_placement=mkEl('center');map.acc_placement_manual=mkEl('0');}
  ...
  if((route==='accident_injury'||route==='accident_below_real')!==!!model.accident) throw new Error('accident mismatch '+route);
  if(model.proceeding.leadCaseNumber!=='CASE-'+route) throw new Error('case mismatch '+route);
}
console.log('PASS routes 9/9');